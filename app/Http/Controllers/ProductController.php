...
class ProductController extends Controller
{
    ...
    public function generateInvoiceBarcode(Invoice $invoice)
    {
        ...
        // Check if `invoice_code` is set
        if (!$barcodeContent) {
            return redirect()->back()->with('error', 'كود الفاتورة مفقود.');
        }
        
        // توليد صورة الباركود كسلسلة base64
        $barcodeImage = $barcodeGenerator->getBarcodePNG($barcodeContent, 'C128');
        ...
        // Pass the path of the saved barcode image to the view
        return view('admin.invoices.generate_barcode', compact('invoice', 'barcodePath'));
    }
