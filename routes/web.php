...
Route::middleware([
   ...
   Route::get('/cashier/search-product', [CashierController::class, 'searchProductByName'])->name('cashier.searchProductByName');
    ...
    Route::get('/print/clients', [App\Http\Controllers\ClientController::class, 'printTable'])->name('clients.printTable');
    Route::get('/clients/{client}/print-statement', [App\Http\Controllers\ClientController::class, 'printStatement'])->name('clients.printStatement');
    Route::resource('villages', App\Http\Controllers\VillageController::class);
    // Custom search route must come before the resource route
    Route::get('/maintenances/search', [App\Http\Controllers\MaintenanceController::class, 'search'])->name('maintenances.search');
    // Resource route for maintenances
    Route::resource('maintenances', App\Http\Controllers\MaintenanceController::class)->except(['show']);
    // Route for printing
    Route::get('/maintenances/{id}/print', [App\Http\Controllers\MaintenanceController::class, 'print'])->name('maintenances.print');
    Route::get('/maintenances/print-table', [App\Http\Controllers\MaintenanceController::class, 'printTable'])->name('maintenances.printTable');
    Route::get('/clients/{client}/maintenance-statement', [App\Http\Controllers\MaintenanceController::class, 'clientMaintenanceStatement'])->name('clients.maintenance.statement');
    Route::get('/maintenance/clients', [App\Http\Controllers\MaintenanceController::class, 'clientIndex'])->name('maintenance.clients.index');
    Route::get('/products/report/sold-products', [App\Http\Controllers\ReportController::class, 'soldProductsReport'])->name('reports.soldProductsReport');
    Route::get('/invoice/scan', [App\Http\Controllers\InvoiceController::class, 'scanBarcode'])->name('invoices.scan');
    Route::post('/invoices/scan', [App\Http\Controllers\InvoiceController::class, 'redirectToInstallmentByBarcode'])->name('invoices.redirectByBarcode');
    Route::get('/sales/installment/daily-summary', [App\Http\Controllers\SalesInstallmentController::class, 'dailySummary'])->name('sales.installments.dailySummary');
    Route::patch('/clients/{client}/update-flags', [App\Http\Controllers\ClientController::class, 'updateFlags'])->name('clients.updateFlags');

    Route::get('/invoices/{invoice}/generate-barcode', [App\Http\Controllers\InvoiceController::class, 'generateInvoiceBarcode'])->name('invoices.generateBarcode');
    ...
    Route::prefix('admin/users')->name('users.')->group(function () {
        ...
        Route::post('/{userId}/assign-role', [App\Http\Controllers\UserRoleController::class, 'assignRole'])->name('assignRole');
        Route::post('/{userId}/remove-role', [App\Http\Controllers\UserRoleController::class, 'removeRole'])->name('removeRole');
        Route::post('/{userId}/add-permission', [App\Http\Controllers\UserRoleController::class, 'addPermission'])->name('addPermission');
        Route::post('/{userId}/remove-permission', [App\Http\Controllers\UserRoleController::class, 'removePermission'])->name('removePermission');
    });
